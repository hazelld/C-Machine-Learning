...
matrix_t* multiply_vector(matrix_t* m, matrix_t* n) {
	...
	init_matrix(res, m->rows, m->columns);
	for (int i = 0; i < m->rows; i++) 
...
matrix_t* subtract_vector(matrix_t* m, matrix_t* n) {
	matrix_t* diff = malloc(sizeof(matrix_t));
	init_matrix(diff, m->rows, m->columns);
	for (int i = 0; i < m->rows; i++) 
...
matrix_t* kronecker_vectors (matrix_t* vec1, matrix_t* vec2) {
	...
	matrix_t* kv = malloc(sizeof(matrix_t));
	init_matrix(kv, rows, columns);
